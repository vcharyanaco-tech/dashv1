...
>
> **Next up:** investigate why warm login is ~2.5-3.4s slower than the older 3.5-4.1s baseline (candidates: apiCall_ retry path, SW network-first fetch cost, GAS container warm-up); consider a profile trace (Stackdriver / Timing.h) on the login `doPost`. Also note: `getAiInsights` only enforces admin when a token is passed — anonymous callers can burn the Groq quota; consider requiring login even for the tokenless path.

## Current Task (AI provider priority chain — DONE, LIVE @236)
Resumed from opencode session `ses_00ab056e...` (export `session-ses_00ab.md`). The session had completed the code (uncommitted) but stopped before push + key config because `clasp run` was blocked by an OAuth/client-project restriction.

### What was completed
1. **Code (commits `0a36078`, `3507ba7`):** `EnterpriseService.gs` — OpenCode Zen provider added (`callOpencode_`, `aiKeyPropName_`/`aiModelPropName_`/`aiDefaultModel_` per-provider helpers), `generateAiText_` rewritten as a priority chain over `AI_PROVIDER_ORDER` (default `groq,opencode,kilo,gemini`; `AI_PROVIDER` override moves a provider to the front; Kilo skipped when `AI_KILO_FALLBACK=false`), errors aggregated per provider. HF/OpenRouter removed repo-wide (verified: zero leftover references). `EnterpriseSettings.js` comments updated.
2. **Key configuration (NEVER committed):** `configureAI` was added to `API_ROUTES` as a TEMPORARY route, pushed, redeployed @234, invoked via the live web app URL with the Groq + OpenCode keys from the session export → Script Properties (`keysSet: [GROQ_API_KEY, OPENCODE_API_KEY]`), then the route was **removed** and redeployed @235/@236.
3. **Live verification (@236):** `configureAI` → `{"error":"Unknown function: configureAI"}` (route gone); `getAiInsights` → `{"success":true,"insights":"...","provider":"groq"}` (chain works, Groq key valid — the earlier 403s were Cloudflare blocking this machine's IP, `error code: 1010`, not the key).
4. **Admin setters added:** `setOpencodeApiKey`, `setAiProvider`, `setAiProviderOrder` (+ one-time `configureAI`, editor/`clasp run` only — doc comment updated to make clear it is NOT admin-gated and must stay out of `API_ROUTES`).
