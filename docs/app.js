...
var API_URL = 'https://script.google.com/macros/s/AKfycbzLwxHpeudnLydvoPmFry1WkRrRayBMuSWd-VqPt6zehFOJocLw1CJqCzHbt3NDOLsJ/exec';
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  th { background: #1f5c2e; color: #fff; font-weight: 600; }
...
<body>
  <div class="report-header">
    ...
    <div class="meta">Generated ${escapeHtml(now)} &middot; ${count} record${count === 1 ? '' : 's'}</div>
  </div>
  <table>
    <thead>
  ...
  const win = window.open('', '_blank', 'width=980,height=720');
  if (!win) { showToast('Pop-up blocked. Please allow pop-ups to print the report.', 'error'); return; }
  win.document.open();
  win.document.write(html);
  win.document.close();
}
...
function saveTask() {
  ...
  const params = {
    ...
    dueDate: getEl('taskDueDate').value,
...
function wireGlobalEvents() {
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Escape') {
      document.querySelectorAll('.review-dropdown-menu.open').forEach(function (menu) {
