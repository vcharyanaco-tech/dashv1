...
function renderTaskList() {
  ...
  if (tbody) {
    tbody.innerHTML = tasks.map(function (t) {
      ...
      let actionButtons = '';
      if (t.status === 'OPEN') {
        actionButtons += '<button class="btn btn-ghost btn-small" type="button" onclick="startTask(\'' + escAttr(t.id) + '\')" style="margin-right:4px;">Start</button>';
      }
...
function closeTaskModal() {
  ...
  // Status is edit-only; new tasks always start OPEN.
  getEl('taskStatusField').classList.add('hidden');
  getEl('taskStatus').value = 'OPEN';
}
...
function saveTask() {
  ...
  if (appState.editingTaskId) {
    // Status can only be changed when editing (new tasks always start OPEN)
    params.status = getEl('taskStatus').value;
...
/* Optimistic "Start" for OPEN tasks: flips the badge to IN_PROGRESS
   immediately, disables the row while in flight, and rolls back on conflict /
   failure — mirroring completeTaskOptimistic. The server accepts OPEN ->
   IN_PROGRESS via updateTaskField (validated state machine). */
function startTask(id) {
  ...
  const rowVersion = known && known.rowVersion ? Number(known.rowVersion) : 1;
  const idempotencyKey = newClientId_();
  ApiService.updateTaskField(id, 'status', 'IN_PROGRESS', rowVersion, idempotencyKey).then(function (res) {
    if (res && res.conflict) {
      // RowVersion conflict — the task changed elsewhere. Do NOT overwrite:
      // revert this row's optimistic state and refetch the fresh list.
      row.innerHTML = snapshot;
    ...
    row.classList.remove('task-pending');
    buttons.forEach(function (b) { b.disabled = false; });
    // The Start action only applies to OPEN tasks — drop its button now that
    // the row shows IN_PROGRESS (Complete/Edit remain, matching the new state).
    ...
    row.classList.remove('task-pending');
    if (handleServerFailure(err)) return;
    showToast('Could not start task: ' + (err.message || err), 'error');
  });
}
...
function editTask(id) {
  ...
  // Reveal the status field (hidden for new tasks) and preselect current status
  getEl('taskStatusField').classList.remove('hidden');
  getEl('taskStatus').value = task.status || 'OPEN';
...
function autoRefreshTick() {
  ...
  ApiService.getAppData().then(function (data) {
    ...
    populateResponsibilitySelect();
    renderDashboard();
    // Review-reminder generation is daily-deduped server-side and runs on login
    // + manual refresh; NOT on every 60s auto-refresh (it iterates all records
    // under a lock, so per-tick calls add server load without new notifications).
    EventBus.emit('DataRefreshed');
  }).catch(function (err) {
    autoRefreshInFlight = false;
    if (handleServerFailure(err)) return;
  ...
  tbody.innerHTML = users.length ? users.map(function (u, i) {
    ...
    return `
      <tr${resetPending ? ' class="row-reset-requested"' : ''}>
        ...
        <td><button class="btn btn-ghost btn-small" type="button" data-action="killSessions" data-index="${i}" title="Sign this user out of all devices">Sign out everywhere</button></td>
...
function killUserSessions(email) {
  return showConfirm({
    title: 'Sign out everywhere',
    message: 'Sign ' + email + ' out of all devices? They will need to sign in again.',
    okLabel: 'Sign out',
    danger: true
  }).then(function (ok) {
    if (!ok) return;
    showOverlay('Signing out ' + email + '…');
    ApiService.adminKillUserSessions(email).then(function (res) {
      hideOverlay();
      const result = res || {};
      showToast(result.message || ('Signed ' + email + ' out everywhere'), 'success');
      // Self-target: the acting admin's own sessions die too — force re-login.
      if (result.reAuth) {
        setAuthToken('');
        try { window.sessionStorage.setItem(STORAGE_REAUTH_MSG, 'You signed yourself out everywhere. Please sign in again.'); } catch (err) {}
        window.location.reload();
      }
    }).catch(function (err) {
      hideOverlay();
...
/* ---------------------------------- Reports ---------------------------------- *//* ---------------------------------- Email report ---------------------------------- *//* ---------------------------------- Email all users (broadcast) ---------------------------------- *//* ---------------------------------- Settings ---------------------------------- *//* ---------------------------------- Record detail dialog ---------------------------------- */
...
function optimisticNewItem_(item) {
  const fieldRows = [
    ['Sector', item.sector], ['Description', item.description], ['Entry Date', item.entryDate],
    ['Action', item.action], ['Responsibility', item.responsibility], ['Review Date', item.reviewDate]
  ];
  const displayFields = fieldRows.filter(function (fr) {
    return String(fr[1] || '').trim() !== '';
  }).map(function (fr) {
    return { label: fr[0], value: String(fr[1] || ''), html: '' };
  });
  return {
    row: 'temp-' + Date.now(),
    id: String((appState.items || []).length + 1),
    sector: item.sector || '',
    ...
    reviewStatus: item.flagged ? 'due' : '',
    displayFields: displayFields,
    linkUrls: item.links ? { action: item.links.action || null } : {},
    linkTexts: {}
  };
}
...
function submitNewItem(item) {
  closeEditModal();
  // Optimistic: render the new record immediately (temp row), then reconcile
  // with the authoritative server response so it shows without the round trip.
  const temp = optimisticNewItem_(item);
  if (temp) {
    appState.items = [temp].concat(appState.items || []);
    appState.summary = optimisticSummary_();
    populateFilters();
    const sorted = sortedItems();
    const idx = sorted.findIndex(function (it) { return String(it.row) === String(temp.row); });
    appState.page = Math.max(1, Math.floor((idx >= 0 ? idx : sorted.length - 1) / PAGE_SIZE) + 1);
    renderDashboard();
  }
  ...
  ApiService.addItem(item).then(function (data) {
    ...
    if (temp) {
      appState.items = (appState.items || []).filter(function (it) { return String(it.row) !== String(temp.row); });
      appState.summary = optimisticSummary_();
      renderDashboard();
    }
    if (handleServerFailure(err)) return;
...
function saveItem(item) {
  if (appState.rowPanelCache) delete appState.rowPanelCache[String(item.row)];
  const snapshotItems = (appState.items || []).slice();
  const snapshotSummary = appState.summary;
...
window.EnterpriseAddons.completeTaskOffline = function (taskId) {
  ...
  if (window.OfflineQueue && window.OfflineQueue.enqueue) {
    const task = (appState.tasks || []).find(function (t) { return String(t.id) === String(taskId); });
    const rowVersion = task && task.rowVersion ? Number(task.rowVersion) : 1;
    // Idempotency key is generated ONCE here and persisted in the queue item,
    // so a replayed completion can never be applied twice.
    window.OfflineQueue.enqueue('updateTaskField', {
      args: [taskId, 'status', 'DONE', rowVersion, newClientId_(), getAuthToken()],
      entityType: 'TASK',
