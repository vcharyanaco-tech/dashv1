...
/* Reads the Tasks sheet once and caches the raw list in CacheService keyed by
 * the tasks generation counter. Every task mutation bumps that counter
 * (invalidateCounts_('tasks')), so cached lists are orphaned instantly and
 * tab loads, KPI counts and "my tasks" share a single sheet read per
 * generation instead of one read per request. */
function cachedTasksList_() {
  flushCountBumps_(); // a read must observe pending mutations: collapse them into one bump first
  const key = 'tasks:v1:g' + countGen_(COUNT_GEN_PROP.TASKS);
  ...
  const values = sh.getRange(2, 1, lastRow - 1, TASK_SHEET_HEADERS.length).getValues();
  const rows = [];
  for (let i = 0; i < values.length; i++) rows.push(taskRecordFromRow_(values[i]));
