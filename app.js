...
/* ---------------------------------- AI Meeting Notes ---------------------------------- */
/* Admin-only: uploads a review-meeting audio recording, transcribes it via
   Groq Whisper and renders structured minutes. Action items become "Create
   task" buttons (never auto-created). */
...
function processMeetingNotes() {
  ...
  if (!fileInput || !fileInput.files || !fileInput.files.length) {
    showToast('Choose an audio recording first.', 'warning');
    return;
  }
...
function renderMeetingMinutes(data) {
  ...
  if (actions.length) {
    html += '<div class="card-ai-head"><span class="card-ai-title">Action items (' + actions.length + ')</span></div>' +
...
function wireGlobalEvents() {
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Escape') {
      ...
      ['editModal', 'aboutModal', 'submissionsModal', 'recordDetailModal', 'editUserModal', 'reviewModal', 'taskModal', 'columnModal', 'commandPalette', 'previewModal', 'linkModal', 'aiInsightsModal', 'meetingNotesModal'].forEach(function (id) {
        const el = getEl(id);
